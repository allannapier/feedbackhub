...
export function ShareModal({ response, form, isOpen, onClose }: ShareModalProps) {
  ...
  const generateImageBlob = async (format: string = 'web'): Promise<Blob> => {
    let apiFeedbackText = response.text || '';
    if (!response.text) {
      if (form.type === 'nps') {
        apiFeedbackText = `Rated us ${response.rating}/10`;
    ...
    return await response_api.blob()
  }
  const shareViaNativeAPI = async () => {
    ...
    try {
      ...
      const imageFile = new File([imageBlob], `testimonial-${response.id}.png`, { type: 'image/png' })
      
      const shareData: ShareData = {
        title: `${form.user?.name || form.title} Testimonial`,
        text: shareText,
      ...
      if (navigator.canShare && navigator.canShare(shareData)) {
        ...
        if (navigator.canShare && navigator.canShare(shareDataWithoutFiles)) {
  ...
  const downloadInstagramImage = async () => {
    try {
      const imageBlob = await generateImageBlob('instagram')
  ...
  const downloadAllAssets = async () => {
    try {
      // For now, we'll download the main image
      // In future, we could create a zip file with multiple formats
      const imageBlob = await generateImageBlob('web')
      const url = URL.createObjectURL(imageBlob)
      const a = document.createElement('a')
      a.href = url
      a.download = `testimonial-${response.id}.png`
      document.body.appendChild(a)
      a.click()
      document.body.removeChild(a)
      URL.revokeObjectURL(url)
      ...
      // Copy caption to clipboard
      await navigator.clipboard.writeText(shareText)
      alert('✅ Testimonial image downloaded and caption copied to clipboard!')
    } catch (error) {
        ...
        <div className="p-6">
          ...
          {imageUrl && (
            <div className="space-y-4">
              ...
              {supportsNativeShare && !showPlatformButtons && (
                <div className="space-y-3">
                  <button
                    onClick={shareViaNativeAPI}
                    disabled={isSharing || !usage?.canShareSocial}
                    className="w-full px-6 py-4 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-lg hover:from-indigo-700 hover:to-purple-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center space-x-3 font-medium text-lg shadow-lg"
                  >
                    {isSharing ? (
                      <>
                  ...
                  <button
                    onClick={() => setShowPlatformButtons(true)}
                    className="text-sm text-gray-600 hover:text-gray-900 underline"
                  >
                    Choose specific platform instead
                  </button>
                </div>
              )}
              {/* Platform-specific buttons (fallback or user choice) */}
              {(!supportsNativeShare || showPlatformButtons) && (
                <div className="space-y-3">
                  {supportsNativeShare && (
                    <button
                      onClick={() => setShowPlatformButtons(false)}
